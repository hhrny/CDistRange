...
/*
Returns the Max Euclidean distance (~geo~ = NULL)

*/
    inline double MaxDist(const Rectangle<dim>& r) const;
    inline double MinDist(const Rectangle<dim>& r) const;
/*
...
/*
Distance: returns the Max Euclidean distance between two rectangles

*/
template <unsigned dim>
inline double Rectangle<dim>::MaxDist(const Rectangle<dim>& r) const
{
    assert(this->del.isDefined);
    assert(r.del.isDefined);
...
/*
Distance: returns the Min Euclidean distance between two rectangles

*/
template <unsigned dim>
inline double Rectangle<dim>::MinDist(const Rectangle<dim>& r) const
{
    assert(this->del.isDefined);
    assert(r.del.isDefined);
    ...
    for(i = 0; i < dim; i ++)
    {
        if((r.min[i] >= min[i] && r.min[i] <= max[i]) || (r.max[i] >= min[i] && r.max[i] <= max[i]) || (min[i] >= r.min[i] && min[i] <= r.max[i]) || (max[i] >= r.min[i] && max[i] <= r.max[i]))
        {
            continue;
        }
        if(r.min[i] - max[i] > min[i] - r.max[i])
        {
            sum += pow((r.min[i] - max[i]), 2);
        }
        else
        {
            sum += pow((min[i] - r.max[i]), 2);
