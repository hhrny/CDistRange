...
bool         seti_intersects[4];           // Indicates an intersection of cell ...
/******************************************************************************
...
/******************************************************************************
  ...
  3.3 SETICompputeLine

  Identifies a column or a row in a grid in dependence of a given position.

 ******************************************************************************/

int SETICompputeLine(double BORDER1 ,double BORDER2, int SPLITS, double POS)
{
...
SETI::SETI(SETIArea AREA, int SPLITS) : suf(0), rtreeFile(0)
{
    ...
    cout<<"head->headerPageNo : "<<header->headerPageNo<<endl;
    ...
    cout<<"head->flPageNo : "<<header->flPageNo<<endl;
    cout<<"Test : 1"<<endl;
    ...
    cout<<"Create and initialize first cell page : begin"<<endl;
    ...
    cout<<"Create and initialize first cell page : end"<<endl;
    ...
    cout<<"Write head and cell page into SmiUpdateFile : begin"<<endl;
    UpdateHeader();
    UpdateCells();
    cout<<"Write head and cell page into SmiUpdageFile  : begin"<<endl;
...
SETI::SETI(SmiFileId FILEID) : suf(0)
{
    ...
    cout<<"Exist page number: "<<suf->GetExistPageNum()<<endl;
...
void SETI::Delete( const ListExpr typeInfo, Word& w )
{
    ...
    //setiPtr->suf->SyncFile();
    setiPtr->suf->Close();
    if(setiPtr->suf->IsOpen()){
        setiPtr->suf->Drop();
    }
    setiPtr->rtreeFile->Close();
    if(setiPtr->rtreeFile->IsOpen()){
        setiPtr->rtreeFile->Drop();
    }
    delete setiPtr;
    
    cout<<"SETI::DELETE"<<endl;
    w.addr = 0;
}
...
void SETI::ReadSETI()
{
    ...
    cout<<"SmiFileId: "<<header->rtreeFileID<<endl;
    cout<<"Number of header: "<<header->headerPageNo<<endl;
    cout<<"Number of cell page: "<<header->cellPageNo<<endl;
    cout<<"Number of splits: "<<header->splits<<endl;
    cout<<"Number of cells: "<<header->numCells<<endl;
    cout<<"Number of entries: "<<header->numEntries<<endl;
    header->tiv.Print(cout);
    cout<<endl;
    // Read frontline -----------------------------------------------------------
...
bool SETI::UpdateSETI()
{
    ...
    if(GetSemaphore())
    {
        cout<<"Get Semaphore failed!"<<endl;
        return false;
    }
    else
    {
...
{
    int    startCol  = SETICompputeLine(AREA.x1, AREA.x2, SPLITS, POS1.x);
    int    startRow  = SETICompputeLine(AREA.y1, AREA.y2, SPLITS, POS1.y);
    ...
    for (int i = 0; i < 4; i++) seti_intersects[i] = false;
    if (intersectsRight)  {p.x = isR->x; p.y = isR->y; seti_intersects[0] = true;}
    if (intersectsLeft)   {p.x = isL->x; p.y = isL->y; seti_intersects[1] = true;}
    if (intersectsTop)    {p.x = isT->x; p.y = isT->y; seti_intersects[2] = true;}
    if (intersectsBottom) {p.x = isB->x; p.y = isB->y; seti_intersects[3] = true;}
    delete isR;
    delete isL;
    delete isT;
    delete isB;
...
int InsertHandle(SETI* SETIPTR, UploadUnit* UNITPTR)
{
    ...
    else
    {
        ...
        {
            // Find cell
            int col =SETICompputeLine(hPtr->area.x1, hPtr->area.x2, hPtr->splits, pos1.x);
            int row =SETICompputeLine(hPtr->area.y1, hPtr->area.y2, hPtr->splits, pos1.y);
            ...
            // write back to updatepage
            page->Write(&pageTiv, sizeof(Interval<Instant> ), sizeof(int));
            ...
            if (seti_intersects[0] == true) pos1.x = segment->pos2.x + tol;
            else if (seti_intersects[1] == true) pos1.x = segment->pos2.x - tol;
            else pos1.x = segment->pos2.x;
            if (seti_intersects[2] == true) pos1.y = segment->pos2.y + tol;
            else if (seti_intersects[3] == true) pos1.y = segment->pos2.y - tol;
        ...
        Word& local, Supplier s)
{
    ...
    while(qp->Received(args[0].addr))
    {
        ...
        if( currentAttr->IsDefined() )
        {
            ...
            if(0 == (insertStreamCounter%10000)){
                cout<<insertStreamCounter<<endl;
                // Update file
                //if(!setiPtr->UpdateSETI()) error[0] = true;
            }
            insertStreamCounter++;
        ...
        Instant TIME1, Instant TIME2)
{
    ...
    int left   = SETICompputeLine(hPtr->area.x1,hPtr->area.x2,hPtr->splits,AREA.x1);
    int right  = SETICompputeLine(hPtr->area.x1,hPtr->area.x2,hPtr->splits,AREA.x2);
    int bottom = SETICompputeLine(hPtr->area.y1,hPtr->area.y2,hPtr->splits,AREA.y1);
    int top    = SETICompputeLine(hPtr->area.y1,hPtr->area.y2,hPtr->splits,AREA.y2);
...
/******************************************************************************
  ...
  8 distancequery operator

  Returns all trajectory pages with type all or part.

  8.1 FindTrjPages-method

  Finds all trajectory pages which the distance to search window is between d1 to d2.

  (modify by hhr)
 ******************************************************************************/

//enum PageType {RPART, RALL};


map<db_pgno_t, PageType>* FindTrjPages( SETI* SETIPTR, SETIArea AREA, double d1, double d2, Instant TIME1, Instant TIME2)
{
    SETIHeader* hPtr = SETIPTR->GetHeader();

    map<db_pgno_t, PageType>* rpages = new map<db_pgno_t, PageType>();
    double maxdist, mindist;

    // Check if SmiUpdateFile is released
    if(SETIPTR->GetSemaphore()) return rpages;

    //d2 > d1
    if(d1 >= d2) return rpages;

    // Lock SmiUpdateFile
    SETIPTR->SetSemaphore(true);


    //modify the search window to maxdist of d2
    AREA.x1 -= d2;
    AREA.x2 += d2;
    AREA.y1 -= d2;
    AREA.y2 += d2;
    ...
    if ((AREA.x2 < hPtr->area.x1) ||
            ...
            (AREA.y2 < hPtr->area.y1) ||
            (AREA.y1 > hPtr->area.y2))
        return rpages; // Search window does not intersect the SETI area
    ...
    Rectangle<2> qrect = Rectangle<2>(true, AREA.x1, AREA.x2, AREA.y1, AREA.y2);
    ...
    for (int i = bottom; i <= top; i++)
    {
        for (int j = left; j <= right; j++)
        {
            ...
            //
            //
            SETIArea *sa = &(SETIPTR->GetCell(j, i)->area);
            Rectangle<2> temprect = Rectangle<2>(true, sa->x1, sa->x2, sa->y1, sa->y2);

            maxdist = qrect.MaxDist(temprect);
            mindist = qrect.Distance(temprect);

            if(maxdist < d2 && mindist > d1){
                //all
                R_TreeLeafEntry<2, TupleId> e;
                if(SETIPTR->GetCell(j, i)->rtreePtr->First(searchBox, e)){
                    rpages->insert(pair<db_pgno_t, PageType>((db_pgno_t)e.info, RALL));
                    while(SETIPTR->GetCell(j, i)->rtreePtr->Next(e)){
                        rpages->insert(pair<db_pgno_t, PageType>((db_pgno_t)e.info, RALL));
                    }
                }
            }
            else if(maxdist < d1 || mindist > d2){
                continue;
            }
            else{
                //part
                R_TreeLeafEntry<2, TupleId> e;
                if(SETIPTR->GetCell(j, i)->rtreePtr->First(searchBox, e)){
                    rpages->insert(pair<db_pgno_t, PageType>((db_pgno_t)e.info, RPART));
                    while(SETIPTR->GetCell(j, i)->rtreePtr->Next(e)){
                        rpages->insert(pair<db_pgno_t, PageType>((db_pgno_t)e.info, RPART));
                    }
                }
...
bool ReadTrjSegFromPage(SETI *SETIPTR, db_pgno_t pageID, list<TrjSeg *> &trjsegments){
    int moID;
    int segID;
    double tivStart;
    double tivEnd;
    ...
    //assert( PageSelected );
    if(! PageSelected){
        cout<<"Warning: can not select the page!\nPageID: "<<pageID<<endl;
        return false;
    }
    ...
    for (int i = 0; i < numSegments; i++)
    {
        ...
        //uptr = new UPoint(false);
        tsptr = new TrjSeg(moID, segID, tivStart, tivEnd, pos1, pos2);
        trjsegments.push_back(tsptr);
    }
    // Drop page
    SETIPTR->GetUpdateFile()->PutPage(page->GetPageNo(),false);
...
SETIArea RectToSETIArea(const Rectangle<2u> &rect)
{
    SETIArea area;
    area.x1 = rect.MinD(0);
    area.x2 = rect.MaxD(0);
...
Rectangle<2u> SETIAreaToRect(const SETIArea &area)
{
    Rectangle<2u> rect = Rectangle<2u>(true, area.x1, area.x2, area.y1, area.y2);
    return rect;
}
...
bool FindCells(SETI* setiptr, Rectangle<2u> qrect, double dist1, double dist2, int condition, queue<SETICell *> &cellsqueue)
{
    int        i,j,left,right,bottom,top;
    SETIHeader *hptr = setiptr->GetHeader();
    SETIArea   area = RectToSETIArea(qrect);
   
    // check if smiupdatefile is released!
    if(setiptr->GetSemaphore()) 
        return false;
    ...
    if(condition == 1 || condition == 4){
        // modify the search window to max distance of dist2
        area.x1 -= dist2;
        area.x2 += dist2;
        area.y1 -= dist2;
        area.y2 += dist2;
        ...
        // find seti cells which belong to the started cell candidates
        left = SETICompputeLine(hptr->area.x1, hptr->area.x2, hptr->splits, area.x1);
        right = SETICompputeLine(hptr->area.x1, hptr->area.x2, hptr->splits, area.x2);
        bottom = SETICompputeLine(hptr->area.y1, hptr->area.y2, hptr->splits, area.y1);
        top = SETICompputeLine(hptr->area.y1, hptr->area.y2, hptr->splits, area.y2);
    }
        ...
        cout<<"condition error in FindCells function!"<<endl;
    }
    //
    for(i = bottom; i <= top; i ++){
        for(j = left; j <= right; j ++){
            if(setiptr->GetCell(j, i) == NULL){
                continue;
            }
            Rectangle<2u> tmprect = SETIAreaToRect(setiptr->GetCell(j, i)->area);
            if(CalcResultType(qrect, tmprect, dist1, dist2, condition) == 0){ // 0: FNONE
                continue;
            }
            cellsqueue.push(setiptr->GetCell(j, i));
        }
    }
    // release smiupdatefile
    setiptr->SetSemaphore(false);
    return (! cellsqueue.empty());
}
...
ListExpr ConvertMP2UUTM(ListExpr args)
{
    ...
    if ( k != 0 )
    {
        ...
        if ( nl->SymbolValue(attrtype2) == UPoint::BasicType()) setiisMPoint = false;
        else setiisMPoint = true;
    }
    else
    {
...
/******************************************************************************
  ...
  14 PrintCellCurrentPage operator

  print the current page of cell (i,j).

  12.1 PrintCellCurrentPageTM - Type mapping method

 ******************************************************************************/
/*
ListExpr PrintCellCurrentPageTM(ListExpr args)
{
    NList type(args);
    if(! type.hasLength(3)){
        return listutlis::typeError("Expecting three arguments.");
    }
}
*/
