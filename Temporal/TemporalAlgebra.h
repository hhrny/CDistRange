...
*/
        ...
        //int PeriodsAtValInterval(const double& value1, const double& value2, vector<Interval<Instant> >& times);
        int PeriodsAtValInterval(const double& value1, const double& value2, Periods& times);

        //int PeriodsAtValB(const double& value, vector<Interval<Instant> >& times);
        int PeriodsAtValB(const double& value, Periods& times);
        //get the periods which value is behind the value;

        //int PeriodsAtValA(const double& value, vector<Interval<Instant> >& times);
        int PeriodsAtValA(const double& value, Periods& times);
        //get the periods which value is above the value;
...
bool Interval<Alpha>::IsValid() const
{
    if( !start.IsDefined() || !end.IsDefined() ){
        //cout<<"start or end time is not defined;"<<endl;
        return false;
    }
