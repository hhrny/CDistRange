...
//
int UReal::PeriodsAtValInterval(const double& value1, const double& value2, Periods &times)
{
    assert(value2 > value1);
    Periods ptemp1(2), ptemp2(2);
    Interval<Instant> interval1, interval2, intervalresult;

    PeriodsAtValA(value1, ptemp1);
    PeriodsAtValB(value2, ptemp2);
    if(ptemp1.IsDefined() == false || ptemp2.IsDefined() ==  false)
    {
        times.SetDefined(false);
        return 0;
    }
    ptemp1.Intersection(ptemp2, times);
    if(! times.IsDefined())
        return 0;
    return times.GetNoComponents();
}
...
//
int UReal::PeriodsAtValA(const double& value, Periods& times)
{
    //get the periods which values is below the param value
    double inst_d[2];
    int no_res = 0;
    bool iscorrect = false;
    DateTime t0(durationtype), t1(instanttype), t2(instanttype);
    Interval<Instant> iv;

    //cout<<"\nPeriodsAtValA begin~"<<endl;
    if( !IsDefined() && value < 0.0)
    {
        times.SetDefined(false);
        return 0;
    }
    ...
    switch(no_res)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            t0.ReadFrom(inst_d[0]);
            t1 = timeInterval.start + t0;
            t0.ReadFrom(inst_d[1]);
            t2 = timeInterval.start + t0;
            if(timeInterval.end.Compare(&t1) < 0 || timeInterval.start.Compare(&t2) > 0){ 
                //end < t1 || start > t2
                times.StartBulkLoad();
                times.Add(timeInterval);
                times.EndBulkLoad();
            ...
            if((timeInterval.start.Compare(&t1) < 0) && timeInterval.Contains(t1)){
                iv = Interval<Instant>(timeInterval.start, t1, true, false);
                times.StartBulkLoad();
                times.Add(iv);
                times.EndBulkLoad();
            }
            if((timeInterval.end.Compare(&t2) > 0) && timeInterval.Contains(t2)){
                iv = Interval<Instant>(t2, timeInterval.end, true, false);
                times.StartBulkLoad();
                times.Add(iv);
                times.EndBulkLoad();
            }
...
int UReal::PeriodsAtValB(const double& value, Periods& times)
{
    //get the periods which values is below the param value
    double inst_d[2];
    int no_res = 0;
    bool iscorrect = false;
    DateTime t0(durationtype), t1(instanttype), t2(instanttype);
    Interval<Instant> tmp_iv, iv;

    //cout<<"\nPeriodsAtValB begin~"<<endl;
    if( !IsDefined() || (r && value < 0.0))
    {
        times.SetDefined(false);
        return 0;
    }
    ...
    switch(no_res)
    {
        case 0:   //should consider the distance is a constant value
        case 1:
            if(Max(iscorrect) < value){
                //times.SetDefined(false);
                //return 0;
                times.StartBulkLoad();
                times.Add(timeInterval);
        ...
        case 2:
            ...
            tmp_iv = Interval<Instant>(t1,t2,true,false);
            if(timeInterval.Intersects(tmp_iv)){
                timeInterval.Intersection(tmp_iv, iv);
                if(iv.IsValid())
                {
                    times.StartBulkLoad();
                    times.Add(iv);
