...
*/
        ...
        R_TreeEntry<dim>* GetEntry(const int index) const;
...
int RTreeLevel::Insert(SmiRecordId id, const Rectangle<3> &box){
    ...
    if(isleaf){
        ...
        cout<<"this is a internal level!"<<endl;
    }
...
int RTreeLevel::Insert(R_TreeNode<3, TupleId> *node){
    int            i; 
    Point3D        p;
    Rectangle<3>   box = node->BoundingBox();
    unsigned long long key;
...
/*
 * if the node to be appended is a leaf node, than copy the node from the one rtree to anthor rtree, 
 * if the node to be appended is a internal node, then append the son node to the rtree, and generate new internal node, append to rtree
 */
    template <unsigned dim, class LeafInfo>
SmiRecordId R_Tree<dim, LeafInfo>::AppendNode(const R_Tree<dim, LeafInfo> *rtree, const R_TreeNode<dim, LeafInfo> *node)
{
    BBox<dim>    bbox;
    SmiRecordId  rid;
    SmiRecord    r;
    bool         flag;
    int          i;
    R_TreeNode<dim, LeafInfo> *newnode;
    R_TreeInternalEntry<dim>  rtreeInternalEntry;
    assert(node);
    if(node->IsLeaf()){
        // the node is leaf node, just append to rtree
        flag = file->AppendRecord(rid, r);
        assert(flag);
        PutNode(r, &node);
        return rid;
    }else{
        // the node is internal node
        newnode = new R_TreeNode<dim, LeafInfo>(false, node->MinEntries(), node->MaxEntries());
        for(i = 0; i < node->EntryCount(); i ++){
            // append the son node to rtree
            R_TreeInternalEntry<dim> *ie = node->GetInternalEntry(i);
            rid = ie->pointer;
            rtreeInternalEntry->box = ie->box;
            R_TreeNode<dim, LeafInfo> *tmp = new R_TreeNode<dim, LeafInfo>(false, rtree->MinInternalEntries(), rtree->MaxInternalEntries());
            rtree->GetNode(rid, *tmp);
            // to append the son node
            rtreeInternalEntry->pointer = AppendNode(rtree, tmp);
            // insert the new entry to new node
            newnode->Insert(rtreeInternalEntry);
        }
        flag = file->AppendRecord(rid, r);
        assert(flag);
        PutNode(r, &newnode);
...
*/
template <unsigned dim, class LeafInfo>
R_TreeEntry<dim>* R_TreeNode<dim,LeafInfo>::GetEntry(const int index) const
{
