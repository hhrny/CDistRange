...
bool SmiUpdateFile::Create(const string& context, uint16_t pageSize)
{
  ...
  if (CheckName(context))
    ...
    cout<<"exist page num: "<<existPageNum<<endl;
...
#if DB_VERSION_REQUIRED(4,6)
		  DbTxn* tid = 0;
      rc = dbMpf->get(&pageno, tid, DB_MPOOL_CREATE, &pagePointer);
#else
...
#if DB_VERSION_REQUIRED(4,6)
      rc = dbMpf->put(pagePointer, DB_PRIORITY_DEFAULT, 0);  //Error: illegal flag specified to DB_MPOOLFILE->put, #The flags parameter is currently unused, and must be set to 0.
#else
      rc = dbMpf->put(pagePointer, 0);
#endif
      SmiEnvironment::SetBDBError(rc);
      rc = dbMpf->sync();
      SmiEnvironment::SetBDBError(rc);
...
bool SmiUpdateFile::AppendNewPage(SmiUpdatePage*& page)
{
  ...
  static long& ctr = Counter::getRef("SmiUpdateFile::AppPage");
...
#if DB_VERSION_REQUIRED(4,6)
      rc = dbMpf->put(pgPt, DB_PRIORITY_DEFAULT, 0);
#else
      rc = dbMpf->put(pgPt, DB_MPOOL_DIRTY);
#endif
    ...
    existPageNum = GetFactPageNum();    //pthread readlock failed: resource deadlock avoided
    return (rc == 0);
}
